...
void FileSystemItem::copyFileFromRealSystem(char *fileName) {
	char buffer;
	int clearSectors = 0;
	int clearBytes = 0;

	FILE *fileSystemSectorsControl;
	fileSystemSectorsControl = fopen(DISK_SECTORS_CONTROL, "r");
	fscanf(fileSystemSectorsControl, "%d", &clearSectors);

	FILE *fileSystem;
	fileSystem = fopen(DISK_NAME, "r+b");
	fseek(fileSystem, 0, SEEK_END);
	long diskSize = ftell(fileSystem);
	...
	nextFileOrFolder[branches] = new FileSystemItem(fileName, inputFileSize, diskSize);
	branches++;
...
void FileSystemItem::copyFileFromVirtualSystem(char *fileName) {
	bool flag = false;
	char buff;
